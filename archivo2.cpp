...
Archivo :: Archivo (Lista<Escritor>* escritor,Lista<Lectura>* lectura, Cola<Lectura>* colaLectura,Hashing<Escritor>* escritorHashing) {

	this -> escritor = escritor;
	this -> lectura = lectura;
	this -> colaLectura = colaLectura;
	this->escritorHashing = escritorHashing;
	this->grafo = new Grafo2<Lectura>;
	//this->escritorHashing = new Hashing<Escritor>;
...
void Archivo :: leerArchivoEscritor() {
    ...
    archivo.open("archivoE.txt", ios::in);
    ...
    if (archivo.fail()){
		...
		int isni = stoi(referencia);
		escritorHashing -> agregarEscritor(nuevoEscritor, isni);
...
void Archivo :: leerArchivoLecturas(){
	...
	archivoLectura.open("archivoL.txt", ios::in);
	...
	if (archivoLectura.fail()){
		...
		while (!archivoLectura.eof()){
			...
			if (narracion == "C"){
				...
				grafo->agregarVertice(nuevoCuento);
			...
			if(narracion == "P"){
				...
				grafo->agregarVertice(nuevoPoema);
			...
			if(narracion == "N"){
				...
				if (datoSegunNarracion == "HISTORICA"){
					...
					grafo->agregarVertice(nuevaHistorica);
					...
					grafo->agregarVertice(nuevaNovela);
