...
Menu :: Menu (Lista<Escritor>* escritores,Lista<Lectura>* lecturas, Cola<Lectura>* colaLectura,Hashing<Escritor>* escritorHashing){
...
/*
void Menu :: armarGrafo(){
	
	Vertice2* nuevoVertice = 0;
	string salida;
	do {
        ...
        for (int i = 1; i <= tamanio; i++) {
            cout << i << ") ";
            lectura = lecturas -> obtenerDato(i)
            nuevoVertice = new Vertice2(lectura);
            lecturas->agregarVertice(nuevoVertice);
            cout << "-------" << endl;
        }
...
void Menu :: mostrarHashing(){
	/*
	for (int i = 0; i <= TAMANIO; i++){
		cout << i << ")";
		escritorHashing->mostrarHashing(i)->mostrarDatos();
	}*/
	escritorHashing->mostrarHashing();
}
